...
function file_exists(file)
	local cmd = "/bin/bash -c 'if [ -e " .. file .. " ]; then echo 
true; fi;'"
...
----@ return string of app path, or nil (remember, only nil and false is 
false in lua)
...
function require_safe(lib)
	if file_exists(awful.util.getdir("config") .. '/' .. lib 
..'.lua') or
		file_exists(awful.util.getdir("config") .. '/' .. lib) 
then
			require(lib)
	end
end
